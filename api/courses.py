...
async def create_course(
    ...
    # Save synchronously so course is available immediately when user refreshes
    try:
        await save_course_to_db(course, current_user.id)
    except Exception as e:
        import logging
        logging.getLogger("sikai").error(f"Course save error: {e}")
        # Don't fail the request if save fails - user still gets the course
    return course
