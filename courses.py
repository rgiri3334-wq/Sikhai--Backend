# ============================================================
#  api/courses.py — Course Generation + CRUD Routes
# ============================================================

from fastapi import APIRouter, Depends, HTTPException, BackgroundTasks
from loguru import logger

from db.models import (
    CourseGenerateRequest, CourseOutline, CourseListItem,
    UserProfile, SuccessResponse,
)
from ai.course_engine import generate_course, save_course_to_db
from services.auth_service import get_current_user
from db.client import get_db

router = APIRouter()


@router.post(
    "/generate",
    response_model=CourseOutline,
    summary="Generate AI course for any topic",
)
async def create_course(
    body: CourseGenerateRequest,
    background_tasks: BackgroundTasks,
    current_user: UserProfile = Depends(get_current_user),
):
    """
    **Main AI endpoint** — generates a complete course.

    - Checks content safety
    - Uses cache if same topic was generated before
    - Calls Groq LLM (Llama 3.3 70B) to generate full course
    - Saves to database in background (non-blocking)

    **Cost:** ~$0.002–0.005 per course (Groq pricing)
    **Speed:** ~3–8 seconds for full course generation
    """
    course = await generate_course(
        topic=body.topic,
        grade=body.grade,
        level=body.level,
        language=body.language,
        user_id=current_user.id,
    )

    # Save to DB in background — don't block the response
    background_tasks.add_task(save_course_to_db, course, current_user.id)

    return course


@router.get(
    "/",
    response_model=list[CourseListItem],
    summary="List user's generated courses",
)
async def list_courses(
    limit: int = 20,
    offset: int = 0,
    current_user: UserProfile = Depends(get_current_user),
):
    """Returns all courses generated by the current user."""
    db = get_db()
    result = (
        db.table("courses")
        .select("id,topic,title,subject,grade,level,total_modules,total_lessons,estimated_hours,created_at")
        .eq("user_id", current_user.id)
        .order("created_at", desc=True)
        .range(offset, offset + limit - 1)
        .execute()
    )
    return result.data or []


@router.get(
    "/{course_id}",
    response_model=CourseOutline,
    summary="Get full course with all lessons",
)
async def get_course(
    course_id: str,
    current_user: UserProfile = Depends(get_current_user),
):
    """
    Fetch a complete course including all modules and lesson content.
    Builds the full nested structure from DB tables.
    """
    db = get_db()

    # Get course
    course_res = db.table("courses").select("*").eq("id", course_id).single().execute()
    if not course_res.data:
        raise HTTPException(status_code=404, detail="Course not found")

    c = course_res.data

    # Verify ownership
    if str(c["user_id"]) != current_user.id:
        raise HTTPException(status_code=403, detail="Not your course")

    # Get modules
    mods_res = (
        db.table("modules").select("*")
        .eq("course_id", course_id)
        .order("module_number")
        .execute()
    )

    modules_data = []
    for m in (mods_res.data or []):
        # Get lessons for this module
        lessons_res = (
            db.table("lessons").select("*")
            .eq("module_id", m["id"])
            .order("lesson_number")
            .execute()
        )

        lessons = [
            {
                "lesson_number": l["lesson_number"],
                "title": l["title"],
                "title_np": l.get("title_np"),
                "content_text": l["content_text"],
                "audio_script": l.get("audio_script", ""),
                "video_script": l.get("video_script"),
                "key_points": l.get("key_points", []),
                "nepal_example": l.get("nepal_example", ""),
                "duration_minutes": l.get("duration_minutes", 10),
            }
            for l in (lessons_res.data or [])
        ]

        modules_data.append({
            "module_number": m["module_number"],
            "title": m["title"],
            "title_np": m.get("title_np"),
            "description": m.get("description", ""),
            "lessons": lessons,
        })

    return CourseOutline(
        id=c["id"],
        topic=c["topic"],
        title=c["title"],
        title_np=c.get("title_np"),
        subject=c.get("subject", "other"),
        grade=c["grade"],
        level=c["level"],
        language=c.get("language", "mixed"),
        description=c.get("description", ""),
        total_modules=c["total_modules"],
        total_lessons=c["total_lessons"],
        estimated_hours=c.get("estimated_hours", 0),
        revision_summary=c.get("revision_summary", ""),
        modules=modules_data,
        created_at=c["created_at"],
    )


@router.delete(
    "/{course_id}",
    response_model=SuccessResponse,
    summary="Delete a course",
)
async def delete_course(
    course_id: str,
    current_user: UserProfile = Depends(get_current_user),
):
    db = get_db()
    result = db.table("courses").select("user_id").eq("id", course_id).single().execute()
    if not result.data:
        raise HTTPException(status_code=404, detail="Course not found")
    if str(result.data["user_id"]) != current_user.id:
        raise HTTPException(status_code=403, detail="Not your course")

    db.table("courses").delete().eq("id", course_id).execute()
    return SuccessResponse(message="Course deleted successfully")
